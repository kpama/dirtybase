...
pub(crate) fn generate_method(
    ...
    let child_model = format_ident!("{}{}", &column.the_type, postfix);
    ...
    let foreign_id = attribute
        .foreign_key
        .clone()
        .unwrap_or_else(|| default_fk_column.to_string());

    let tbl_name = if attribute.foreign_tbl.is_some() {
        let name = attribute.final_tbl.as_ref().unwrap();
        quote! { #name }
    } else {
        quote! {<#the_type as TableEntityTrait>::table_name()}
    };

    // Method
    quote! {

      pub fn #method_name(&mut self) -> #child_model {
        ...
        #child_model::new_with_builder(std::sync::Arc::clone(&self.manager),self.#prop_name.clone().unwrap())
      }
    }
}
