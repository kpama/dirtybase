...
pub(crate) fn generate_join_method(
    ...
    if let Some(RelType::BelongsTo { attribute }) = &attr.relation {
        ...
        // parent key
        let mut parent_key = quote! {<#foreign_type as ::dirtybase_contract::db_contract::table_model::TableModel>::foreign_id_column()};
        // foreign key
        let mut foreign_key = quote! { <#foreign_type  as ::dirtybase_contract::db_contract::table_model::TableModel>::id_column() };
