...
pub(crate) fn generate_join_method(
    ...
    if let Some(RelType::BelongsTo { attribute }) = &attr.relation {
        ...
        // local column
        let mut parent_col = quote! {<#foreign_type as ::dirtybase_shared_type::db::table_model::TableModel>::foreign_id_column()};
        // parent column
        let mut foreign_col = quote! { <#foreign_type  as ::dirtybase_shared_type::db::table_model::TableModel>::id_column() };
