...
impl dirtybase_contract::ExtensionSetup for Extension {
    ...
    fn register_routes(
        &self,
        manager: &mut RouterManager,
    ){
        http::register_routes(manager);
    }


    async fn on_cli_command(&self, cmd: &str, matches: ArgMatches, _context: Context) -> ArgMatches {
