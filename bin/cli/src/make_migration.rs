...
pub fn make(name: &str, path_buf: &std::path::PathBuf, module_name: &str) {
    let built = MIGRATION_STUB.replace("struct_name", name);

    // TODO: Check if migration folder exist
    let migration_dir = path_buf.join("migration");
    if !migration_dir.exists() {
        _ = std::fs::create_dir(migration_dir);
    }
    // TODO: Check if migration module exist
    let mod_path = path_buf.join("migration.rs");
    if !mod_path.exists() {
    ...
    module = module.replace(
        "// dty_inject",
        format!("Box::new({}::{}),\n // dty_inject", module_name, name).as_str(),
    );
