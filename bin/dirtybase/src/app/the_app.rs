...
use super::event_subscribers::register_listeners;
use super::setup_database::create_data_tables;
use super::setup_defaults::setup_default_entities;
use super::{Config, EventDispatcher, EventDispatcherBuilder};
use actix_web::web;
...
impl DirtyBase {
    pub async fn new(config: Config) -> anyhow::Result<Self> {
        ...
        let event_dispatcher = register_listeners(EventDispatcherBuilder::new())
            .build()
            .await;
    ...
    pub fn dispatch(&self, name: &str, event: ColumnAndValue) {
        log::error!("dispatching event: {}", name);
        self.event_dispatcher.dispatch(name, event);
    }
