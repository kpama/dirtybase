...
struct ResourceWrapper<T: Clone + Send + Sync + 'static> {
    resource: T,
    last_ts: AtomicI64,
    idle_timeout: i64, // In seconds
}

impl<T: Clone + Send + Sync + 'static> ResourceWrapper<T> {
...
type SetupFn = Arc<
    RwLock<
        Box<
            dyn FnMut(Context) -> BoxFuture<'static, Result<ResourceManager, anyhow::Error>>
                + Send
                + Sync,
        >,
    >,
...
mod test {
    ...
    async fn test_context_creation() {
        let manager = ContextResourceManager::new(
            |_| Box::pin(async { Ok(("global".to_owned(), 20).into()) }),
    ...
    async fn test_idle_time_expired() {
        let manager = ContextResourceManager::new(
            |_| Box::pin(async { Ok(("counter", 100).into()) }),
            |_| Box::pin(async { Ok(100) }),
            |_| Box::pin(async {}),
        )
        .await;
    ...
    async fn test_idle_time_not_expired() {
        let manager = ContextResourceManager::new(
            |_| Box::pin(async { Ok(("counter", 20).into()) }),
            |_| Box::pin(async { Ok(100) }),
            |_| Box::pin(async {}),
        )
        .await;
